...
# Read in Data
dataset <- read.table("/Users/Emma/Downloads/MI.data", sep = ",", dec = ".")
dataset <- dataset %>%
...
dataset <- dataset %>% select(-c("KFK_BLOOD", "IBS_NASL",	"S_AD_KBRIG",	"D_AD_KBRIG",	"NOT_NA_KB",	"LID_KB",
                                 "NA_KB",	"GIPER_Na",	"Na_BLOOD",	"K_BLOOD",	"GIPO_K",	"AST_BLOOD",	"ALT_BLOOD",
...
tree_mod.cc<-tree(death~., temp_train,
                  minsize=2, 
                  model=T)

tree_pred.cc<-predict(tree_mod.cc, newdata=temp_test, type = "class")
tree.cc.error <- 1 - sum(diag(table(temp_test$death, tree_pred.cc)))/length(temp_test$death)
...
tree_mod.mi<-tree(death~., temp_train.mi,
                  minsize=2, 
                  model=T)


tree_pred.cc<-predict(tree_mod.mi, newdata=temp_test.mi, type = "class")
tree.error <- 1 - sum(diag(table(temp_test.mi$death, tree_pred.cc)))/length(temp_test.mi$death)
...
# #################
rpart.tree.surrogate <- rpart(death ~ ., data = temp_train.mi,  method = "class" ,
                              control = rpart.control(usesurrogate = 1))

rpart.pred.surrogate<-predict(rpart.tree.surrogate,temp_test.mi, type = "class")
rpart.error.surrogate <- 1 - sum(diag(table(temp_test.mi$death, rpart.pred.surrogate)))/length(temp_test.mi$death)

rpart.tree.surrogate.2 <- rpart(death ~ ., data = temp_train.mi,  method = "class" ,
                                control = rpart.control(usesurrogate = 2))

summary(rpart.tree.surrogate.2)

rpart.pred.surrogate.2<-predict(rpart.tree.surrogate.2,temp_test.mi, type = "class")
rpart.error.surrogate.2 <- 1 - sum(diag(table(temp_test.mi$death, rpart.pred.surrogate.2)))/length(temp_test.mi$death)

# ###############
# Random Forest - na.action = na.roughfix
# ###############
# Complete Case analysis
rand_forest<-randomForest(as.factor(death) ~., data=temp_train, ntree=200,  
                          nodesize=1, importance=T, 
                          type='classification')
rand_forest_pred<-predict(rand_forest, newdata=temp_test, type = "class")
rand.forst.error <- 1 - sum(diag(table(temp_test$death, rand_forest_pred)))/length(temp_test$death)

# With missing data - na.roughfix
rand_forest.mi <-randomForest(as.factor(death) ~., data=temp_train.mi, ntree=200,  
                              nodesize=1, importance=T, 
...
rand.forst.error <- 1 - sum(diag(table(na.omit(temp_test.mi)$death, rand_forest_pred)))/length(na.omit(temp_test.mi)$death)

# With missing data - proximity
temp_train.mi$death <- as.factor(temp_train.mi$death )
rand.forest.imputed <- rfImpute(death ~., data=temp_train.mi, ntree=200)
rf.imputed <- randomForest(death  ~., data=rand.forest.imputed)
rf.imputed.pred<-predict(rf.imputed, newdata=na.omit(temp_test.mi), type = "class")
rf.imputed.error <- 1 - sum(diag(table(na.omit(temp_test.mi)$death, rf.imputed.pred)))/length(na.omit(temp_test.mi)$death)
...
adaboost <- gbm(death ~., data=temp_train,
...
adaboost_pred<-ifelse(
  ...
  1,0)
adaboost.error <- 1 - sum(diag(table(na.omit(temp_test)$death, adaboost_pred)))/length(na.omit(temp_test)$death)
